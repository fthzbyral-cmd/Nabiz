    ...
    <div style={{ background: '#07090f', minHeight: '100vh', color: '#fff', fontFamily: 'sans-serif', maxWidth: 480, margin: '0 auto' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(5,1fr)', gap: 5, padding: '0 14px 6px' }}>
        ...
        <div style={{ background: '#0e1520', borderRadius: 10, padding: '8px 4px', textAlign: 'center', border: '.5px solid #1a2535' }}>
          ...
          <div style={{ fontSize: 8, fontWeight: 700, color: '#fff' }}>{ (Object.entries(results?.byProvince || {}).sort((a:any,b:any)=>Object.values(b[1]).reduce((s:any,v:any)=>s+v,0)-Object.values(a[1]).reduce((s:any,v:any)=>s+v,0))[0]?.[0] || 'YOK').toUpperCase()}</div>
          <div style={{ fontSize: 7, color: '#445' }}>{ Object.entries(results?.byProvince || {}).sort((a:any,b:any)=>Object.values(b[1]).reduce((s:any,v:any)=>s+v,0)-Object.values(a[1]).reduce((s:any,v:any)=>s+v,0))[0]?.[1] ? Object.values(Object.entries(results?.byProvince || {}).sort((a:any,b:any)=>Object.values(b[1]).reduce((s:any,v:any)=>s+v,0)-Object.values(a[1]).reduce((s:any,v:any)=>s+v,0))[0]?.[1] as any).reduce((s:any,v:any)=>s+v,0) : 0} kişi</div>
        </div>
        <div style={{ background: '#0e1520', borderRadius: 10, padding: '8px 4px', textAlign: 'center', border: '.5px solid #1a2535' }}>
